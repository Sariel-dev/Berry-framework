...
local shopsDisabled = true

if shopsDisabled then
	lib.callback.register('ox_inventory:openShop', function()
		return
	end)

	lib.callback.register('ox_inventory:buyItem', function()
		return false, false, { type = 'error', description = 'Shops are disabled on this server.' }
	end)
...
local function canAffordItem(inv, currency, price)
	...
	local balance = currencyItem and Inventory.GetItemCount(inv, currency) or 0

	if not currencyItem then
		local player = server.GetPlayerFromId and server.GetPlayerFromId(inv.id)
	...
	local currencyLabel = currency == 'money' and locale('$')
		or (currencyItem and currencyItem.label)
		or currency

	return canAfford or {
		type = 'error',
		description = locale('cannot_afford', ('%s%s'):format((currency == 'money' and locale('$') or math.groupdigits(price)), (currency == 'money' and math.groupdigits(price) or (' '..currencyLabel))))
	}
end
...
local function removeCurrency(inv, currency, price)
	...
	if player and player.removeAccountMoney then
		player.removeAccountMoney(currency, price)
	end
end
...
lib.callback.register('ox_inventory:buyItem', function(source, data)
	...
	if type(data.count) ~= 'number' or data.count < 1 or data.count > 10000 then return end
	if type(data.fromSlot) ~= 'number' or data.fromSlot < 1 then return end
	if data.toSlot and (type(data.toSlot) ~= 'number' or data.toSlot < 1) then return end
	...
	local shop = shopId and Shops[shopType] and Shops[shopType][shopId] or Shops[shopType]
	if not shop or not shop.items then return end
	
	local fromData = shop.items[data.fromSlot]
	if not fromData then return end
	
	local toData = data.toSlot and playerInv.items[data.toSlot] or nil
	...
	-- Handle gender-specific clothing for female players
	if playerInv.player and not playerInv.player.sex then  -- sex = false/0 for female, true/1 for male
		-- Check if shop item data has gender-specific metadata
		if fromData.metadata and fromData.metadata.female then
			metadata.drawable = fromData.metadata.female
			if fromData.metadata.texture then
				metadata.texture = fromData.metadata.texture
			end
			if fromData.metadata.component then
				metadata.component = fromData.metadata.component
			end
		elseif type(fromItem.drawable) == 'table' and fromItem.drawable.female then
			-- Also check item definition for gender variants
			metadata.drawable = fromItem.drawable.female
			if type(fromItem.texture) == 'table' and fromItem.texture.female then
				metadata.texture = fromItem.texture.female
			end
		end
	end
	...
	if toData == nil or (fromItem.name == toItem?.name and fromItem.stack and table.matches(toData.metadata, metadata)) then
		...
		local currencyLabel = currency == 'money' and locale('$')
			or (currencyItem and currencyItem.label)
			or currency
		local message = locale('purchased_for', count, metadata?.label or fromItem.label, (currency == 'money' and locale('$') or math.groupdigits(price)), (currency == 'money' and math.groupdigits(price) or (' '..currencyLabel)))
