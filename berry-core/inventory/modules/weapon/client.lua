...
function Weapon.Equip(item, data, noWeaponAnim)
	...
	if item.group == `GROUP_PETROLCAN` or item.group == `GROUP_FIREEXTINGUISHER` then
		item.metadata.ammo = item.metadata.ammo or 100
		SetPedInfiniteAmmo(playerPed, true, data.hash)
	end

	TriggerEvent('ox_inventory:currentWeapon', item)

	if client.itemnotify then
		Utils.ItemNotify({ item, 'weapon_equip', 1 })
	end
...
function Weapon.Disarm(currentWeapon, noAnim)
	if currentWeapon and currentWeapon.timer then
		...
		local itemData = Items[currentWeapon.name]
		if client.itemnotify and itemData then
			Utils.ItemNotify({ currentWeapon, 'weapon_disarm', 1 })
		end
