...
local function loadESXPlayers()
	if not server.setPlayerInventory then
		SetTimeout(100, loadESXPlayers)
		return
	end
	...
	if customInventory ~= nil and customInventory ~= "ox" then
        error('esx has not been configured to enable support for ox_inventory!\nEnsure Config.CustomInventory has been set to "ox" in your esx resource config.')
    end
...
function server.setPlayerData(player)
	...
	if player.name then
		local nameParts = {}
		for part in string.gmatch(player.name, "%S+") do
