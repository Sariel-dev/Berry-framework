	...
	if data.type == 'trunk' or data.type == 'glovebox' then
		...
		if not inventory then
			...
			if data.netid then
				...
				if not (entity > 0 and DoesEntityExist(entity)) then
					return false
				end
...
local function isClothingItemForNotify(slotItem)
	...
	if clothingNotifyBlockedItems[slotItem.name] then
		return true
	end
...
---@param action string
---@param count number
local function sendItemNotify(inventoryId, slotItem, action, count)
	if isClothingItemForNotify(slotItem) then
		return
	end

	TriggerClientEvent('ox_inventory:itemNotify', inventoryId, { slotItem, action, count })
end
...
function Inventory.SetSlot(inv, item, count, metadata, slot)
	...
	if currentSlot and newCount < 1 then
		sendItemNotify(inv.id, currentSlot, 'ui_removed', currentSlot.count)
		currentSlot = nil
	else
		currentSlot = {name = item.name, label = item.label, weight = item.weight, slot = slot, count = newCount, description = item.description, metadata = metadata, stack = item.stack, close = item.close, rarity = item.rarity}
		local slotWeight = Inventory.SlotWeight(item, currentSlot)
		currentSlot.weight = slotWeight
		newWeight += slotWeight

		sendItemNotify(inv.id, currentSlot, count < 0 and 'ui_removed' or 'ui_added', math.abs(count))
	end
...
local function generateItems(inv, invType, items)
	...
	for i = 1, #items do
		...
		if not item then
			...
			returnData[i] = {name = item.name, label = item.label, weight = weight, slot = i, count = count, description = item.description, metadata = metadata, stack = item.stack, close = item.close, rarity = item.rarity}
		end
	end
	...
	if result then
		...
		for _, v in pairs(result) do
			...
			if item then
				...
				weight += slotWeight
				returnData[v.slot] = {name = item.name, label = item.label, weight = slotWeight, slot = v.slot, count = v.count, description = item.description, metadata = v.metadata, stack = item.stack, close = item.close, rarity = item.rarity}
			end
		end
	end
...
function Inventory.SetPermanentWeaponState(inv, slotId, state)
	inv = Inventory(inv) --[[@as OxInventory]]
	local slot = inv and inv.items[slotId]
	...
	if not item or not item.weapon then
		return false, 'not_weapon'
	end

	local metadata = table.clone(slot.metadata or {})
	metadata.permanent = state and true or nil

	Inventory.SetMetadata(inv, slotId, metadata)
	return true
end

exports('SetPermanentWeaponState', Inventory.SetPermanentWeaponState)
	...
	if count > 0 then
		...
		if inv.player then
			if slot then
				...
				if slotItem and slotItem.metadata and slotItem.metadata.permanent then
					return false, 'permanent_item'
				end
			else
				local removableCount = 0
				for k, v in pairs(itemSlots) do
					local slotItem = inv.items[k]
					if slotItem and not (slotItem.metadata and slotItem.metadata.permanent) then
						removableCount += v
					end
				end

				if removableCount < count then
					return false, 'permanent_item'
				end
			end
		end
		...
		elseif itemSlots and totalCount > 0 then
			for k, v in pairs(itemSlots) do
				...
				if removed < total then
					if v == count then
						sendItemNotify(inv.id, inv.items[k], 'ui_removed', v)
...
---@param metadata { [string]: any }
---@param slot number
RegisterServerEvent('ox_inventory:removeItem', function(slot, count)
	local source = source
	...
	if type(count) ~= 'number' or count < 1 or count > 10000 then return end
	...
	Inventory.RemoveItem(inventory, item.name, count, nil, slot)
end)
...
local function findNearbyDropId(coords, instance, radius)
	if not coords then
		return nil
	end
	...
	for dropId, dropData in pairs(Inventory.Drops) do
		local dropCoords = dropData and toVec3(dropData.coords)
		if dropData and dropCoords and dropData.instance == instance then
			local dist = #(dropCoords - coords)
			if dist <= radius then
				return dropId
			end
		end
...
local function dropItem(source, playerInventory, fromData, data)
	...
	local inventory
	local mergedIntoExisting = false
	local nearbyDropId = findNearbyDropId(data.coords, data.instance, 1.5)

	if nearbyDropId then
		local nearbyInventory = Inventory(nearbyDropId)
		if nearbyInventory and (nearbyInventory.weight + toData.weight) <= nearbyInventory.maxWeight then
			local added = Inventory.AddItem(nearbyInventory, toData.name, toData.count, toData.metadata, data.toSlot)
			if added then
				dropId = nearbyDropId
				inventory = nearbyInventory
				mergedIntoExisting = true
				if Inventory.Drops[dropId] then
					Inventory.Drops[dropId].items = inventory.items
					Inventory.Drops[dropId].slots = inventory.slots
				end
			end
		end
	end

	if not mergedIntoExisting then
		inventory = Inventory.Create(dropId, ('Drop %s'):format(dropId:gsub('%D', '')), 'drop', shared.dropslots, toData.weight, shared.dropweight, false, {[data.toSlot] = toData})
	...
	if mergedIntoExisting then
		if server.loglevel > 0 then
			lib.logger(playerInventory.owner, 'swapSlots', ('%sx %s transferred from "%s" to existing drop "%s"'):format(data.count, toData.name, playerInventory.label, dropId))
		end
	else
		TriggerClientEvent('ox_inventory:createDrop', -1, dropId, Inventory.Drops[dropId], playerInventory.open and source, slot)
	end

	if server.loglevel > 0 and not mergedIntoExisting then
...
lib.callback.register('ox_inventory:swapItems', function(source, data)
	...
	local toInventory = data.toInventoryId and Inventory(data.toInventoryId)
		or (data.toType == 'player' and playerInventory)
		or Inventory(playerInventory.open)
	local fromInventory = data.fromInventoryId and Inventory(data.fromInventoryId)
		or (data.fromType == 'player' and playerInventory)
		or Inventory(playerInventory.open)

	if fromInventory == playerInventory and toInventory == playerInventory then
		local fromSlot = tonumber(data.fromSlot)
		local toSlot = tonumber(data.toSlot)
		if (fromSlot and fromSlot >= 70 and fromSlot <= 86) or (toSlot and toSlot >= 70 and toSlot <= 86) then
			return false
		end
	end
	...
	if type(fromInventory.items) ~= 'table' or type(toInventory.items) ~= 'table' then
		playerInventory:closeInventory()
		return false
	end
	...
	if toInventory and (data.toType == 'newdrop' or fromInventory ~= toInventory or data.fromSlot ~= data.toSlot) then
		...
		if fromData.metadata and fromData.metadata.permanent and fromInventory.id ~= toInventory.id then
			return false
		end
				...
				if not sameInventory then
					if (toWeight <= toInventory.maxWeight and fromWeight <= fromInventory.maxWeight) then
						...
						if fromOtherPlayer then
							sendItemNotify(fromInventory.id, fromData, 'ui_removed', fromData.count)
							sendItemNotify(fromInventory.id, toData, 'ui_added', toData.count)
						elseif toOtherPlayer then
							sendItemNotify(toInventory.id, fromData, 'ui_added', fromData.count)
							sendItemNotify(toInventory.id, toData, 'ui_removed', toData.count)
						end
			...
			elseif toData and toData.name == fromData.name and table.matches(toData.metadata, fromData.metadata) then
				...
				if fromInventory.type == 'container' or sameInventory or totalWeight <= toInventory.maxWeight then
					...
					if not sameInventory then
						...
						if fromOtherPlayer then
							sendItemNotify(fromInventory.id, fromData, 'ui_removed', data.count)
						elseif toOtherPlayer then
							sendItemNotify(toInventory.id, toData, 'ui_added', data.count)
						end
				...
				if fromInventory.type == 'container' or sameInventory or (toInventory.weight + toData.weight <= toInventory.maxWeight) then
					...
					if not sameInventory then
						...
						if fromOtherPlayer then
							sendItemNotify(fromInventory.id, fromData, 'ui_removed', data.count)
						elseif toOtherPlayer then
							sendItemNotify(toInventory.id, fromData, 'ui_added', data.count)
						end
...
local function saveInventories(clearInventories)
    ...
    if total[5] > 0 then
		...
		if not ok and err then return end
    end
...
local function giveItem(playerId, slot, target, count)
	...
	if type(slot) ~= 'number' or slot < 1 or slot > fromInventory.slots then return end
	...
	if toInventory?.player then
		...
		if data.metadata and data.metadata.permanent then
...
local function updateWeapon(source, action, value, slot, specialAmmo)
	...
	local validActions = { 'load', 'throw', 'component', 'ammo', 'melee' }
	...
	if not isValidAction then return end

	local valueType = type(value)

	if valueType == 'table' and action == 'component' then
		if not value.slot or type(value.slot) ~= 'number' then return end
		if value.slot < 1 or value.slot > inventory.slots then return end
		if not value.component or type(value.component) ~= 'string' then return end
		...
		if not slot or slot < 1 or slot > inventory.slots then return end
		...
		if weapon and weapon.metadata then
			...
			if action == 'load' then
				if type(value) ~= 'number' or value < 0 or value > 10000 then return end
				
				local ammo = Items(weapon.name).ammoname
				local diff = value - (weapon.metadata.ammo or 0)
				
				if diff < 0 or diff > 10000 then return end
			...
			elseif action == 'component' then
				if valueType == 'number' then
					if value < 1 or not weapon.metadata.components or not weapon.metadata.components[value] then return end
				...
				elseif valueType == 'string' then
					...
					if not componentSlot or componentSlot < 1 or componentSlot > inventory.slots then return false end
					...
					if not weapon.metadata.components then
						weapon.metadata.components = {}
					end
			...
			elseif action == 'ammo' then
				...
				if item.hash == `WEAPON_FIREEXTINGUISHER` or item.hash == `WEAPON_PETROLCAN` or item.hash == `WEAPON_HAZARDCAN` or item.hash == `WEAPON_FERTILIZERCAN` then
					weapon.metadata.ammo = math.floor(value)
			...
			weapon.metadata.durability = nil
			weapon.metadata.degrade = nil
...
RegisterNetEvent('ox_inventory:updateWeapon', function(action, value, slot, specialAmmo)
	...
	if action and type(action) ~= 'string' then return end
	if slot and (type(slot) ~= 'number' or slot < 1 or slot > inventory.slots) then return end
	if value and type(value) ~= 'number' and type(value) ~= 'table' then return end
...
lib.callback.register('ox_inventory:removeAmmoFromWeapon', function(source, slot)
	...
	if not slotData or not slotData.metadata or not slotData.metadata.ammo or slotData.metadata.ammo < 1 then return end

	local item = Items(slotData.name)

	if not item or not item.ammoname then return end
	
	if slotData.metadata.ammo > 10000 then return end
...
lib.callback.register('ox_inventory:packAmmo', function(source, slot)
	local inventory = Inventory(source)
	...
	-- Mapping between ammo types and their ammo boxes
	local ammoTypeToBox = {
		-- Items-based ammo
		['ammo_pistol'] = 'ammobox_pistol',
		['ammo_rifle'] = 'ammobox_rifle',
		['ammo_shotgun'] = 'ammobox_shotgun',
		['ammo_snip'] = 'ammobox_snip',
		['ammo_sub'] = 'ammobox_sub',
		['ammo_heavy'] = 'ammobox_heavy',
		['ammo_flare'] = 'ammobox_flare',
		['ammo_musquet'] = 'ammobox_musquet',
		['ammo_rocket'] = 'ammobox_rocket',
		['ammo_beambag'] = 'ammobox_beambag',
		
		-- Weapon-based ammo
		['ammo-9'] = 'ammobox_pistol',
		['ammo-45'] = 'ammobox_pistol',
		['ammo-rifle'] = 'ammobox_rifle',
		['ammo-shotgun'] = 'ammobox_shotgun',
		['ammo-sniper'] = 'ammobox_snip',
		['ammo-heavysniper'] = 'ammobox_snip',
		['ammo-grenade'] = 'ammobox_rocket',
		['ammo-rocket'] = 'ammobox_rocket',
		['ammo-laser'] = 'ammobox_global',
		['ammo-flare'] = 'ammobox_flare',
		['ammo-musket'] = 'ammobox_musquet',
		['ammo-5.7'] = 'ammobox_pistol',
		['ammo-40'] = 'ammobox_pistol',
		['ammo-emp'] = 'ammobox_rocket',
	}

	-- Ammo pack limits: 60 for heavy, 24 for others
	local ammoPackLimits = {
		['ammobox_pistol'] = 24,
		['ammobox_rifle'] = 60,
		['ammobox_shotgun'] = 24,
		['ammobox_snip'] = 60,
	...
	local boxType = ammoTypeToBox[slotData.name]
	if not boxType then return end

	-- Get the count of items to pack (respecting ammo pack limits)
	local maxCount = ammoPackLimits[boxType] or 60
	local countToPack = math.min(slotData.count or 1, maxCount)
	
	-- Remove the ammo items
	if Inventory.RemoveItem(inventory, slotData.name, countToPack, slotData.metadata, slot) then
		-- Add the ammo box with metadata containing the ammo type and quantity
		if Inventory.AddItem(inventory, boxType, 1, { ammoType = slotData.name, ammoCount = countToPack, customLabel = slotData.name .. ' x' .. countToPack }) then
			inventory:syncSlotsWithPlayer(nil, inventory.weight)
			if server.syncInventory then server.syncInventory(inventory) end
			return true
		else
			-- If adding box failed, return the ammo items
			Inventory.AddItem(inventory, slotData.name, countToPack, slotData.metadata)
			inventory:syncSlotsWithPlayer(nil, inventory.weight)
			if server.syncInventory then server.syncInventory(inventory) end
			return false
		end
...
lib.callback.register('ox_inventory:unpackAmmoBox', function(source, slot)
	local inventory = Inventory(source)
	...
	-- Check if it's an ammo box
	local isAmmoBox = string.match(slotData.name, '^ammobox_')
	if not isAmmoBox or not slotData.metadata or not slotData.metadata.ammoType or not slotData.metadata.ammoCount then
		return false
	end

	local ammoType = slotData.metadata.ammoType
	local ammoCount = slotData.metadata.ammoCount

	-- Remove the ammo box
	if Inventory.RemoveItem(inventory, slotData.name, 1, slotData.metadata, slot) then
		-- Add the ammo items back
		if Inventory.AddItem(inventory, ammoType, ammoCount) then
			inventory:syncSlotsWithPlayer(nil, inventory.weight)
			if server.syncInventory then server.syncInventory(inventory) end
			return true
		else
			-- If adding items failed, return the box
			Inventory.AddItem(inventory, slotData.name, 1, slotData.metadata)
			inventory:syncSlotsWithPlayer(nil, inventory.weight)
			if server.syncInventory then server.syncInventory(inventory) end
			return false
		end
