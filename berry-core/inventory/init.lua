...
local configFile = LoadResourceFile(GetCurrentResourceName(), 'config_loader.lua')
if configFile then
    local configLoader, err = load(configFile, '@@ox_inventory/config_loader.lua')
    if configLoader and not err then
        configLoader()
    else
    end
else
...
if IsDuplicityVersion() then
    ...
    if not client then
        ...
        if not client.evidencemarker then
        ...
        if not client.craftingmarker then
...
if not LoadResourceFile(shared.resource, 'ui/index.html') and not LoadResourceFile(shared.resource, 'inventory/web/build/index.html') and not LoadResourceFile(shared.resource, 'web/build/index.html') then
    return spamError(
