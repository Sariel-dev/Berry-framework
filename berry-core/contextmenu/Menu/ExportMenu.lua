...
local function BuildItemData(item)
	if (not item or not item.contextType) then
		return nil
	end

	if (item.contextType == "scroll" or item.contextType == "page") then
		return nil
	end

	local title = nil
	if (item.text and item.text.title ~= nil) then
		title = item.text.title
	end

	local rightText = nil
	if (item.rightText and item.rightText.title ~= nil) then
		rightText = item.rightText.title
	end
	...
	if (item.submenu) then
		data.submenu = BuildMenuData(item.submenu)
	end
...
function BuildMenuData(menu)
	...
	local items = {}
	for i, item in ipairs(menu.objectList or {}) do
		local data = BuildItemData(item)
		if (data) then
	...
	return {
		width = menu.width or 0.15,
		items = items
	}
end

function ContextMenuNuiClose()
	if (not ContextMenuUseNui or not nuiOpen) then
		return
	end

	SendNUIMessage({ action = "contextmenu:close" })
...
local function ContextMenuNuiOpen(position)
	if (not ContextMenuUseNui) then
		return
	end

	local payload = BuildMenuData(exportMenu)
	if (not payload) then
		return
	end
...
function InitExportMenu()
	...
	exportMenuPool.OnInteract = function(screenPosition, hitSomething, worldPosition, hitEntity, normalDirection)
		...
		if (ContextMenuUseNui) then
			ContextMenuNuiOpen(screenPosition)
		end
	end
end
...
local function LeftIcon(itemId, icon)
	assert(itemId ~= nil and itemId >= 0 and itemId <= #itemList, "Parameter \"itemId\" must be a valid item id!")
	itemList[itemId].icon = icon
end
...
RegisterNUICallback("contextmenu:select", function(data, cb)
	local itemId = data and tonumber(data.id) or nil
	local item = itemId and itemList[itemId] or nil

	if (item and item.contextType ~= "submenu" and item.contextType ~= "separator" and item.contextType ~= "text") then
...
RegisterNUICallback("contextmenu:close", function(_, cb)
	if (exportMenuPool) then
		exportMenuPool:CloseAllMenus()
	end
